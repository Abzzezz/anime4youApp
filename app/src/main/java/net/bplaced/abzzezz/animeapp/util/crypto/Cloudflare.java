...
/**
 * Taken & modified from: https://stackoverflow.com/questions/43453491/how-to-bypass-cloudflare-ddos-or-redirect-after-5-seconds-using-jsoup
 */
public class Cloudflare {
    ...
    public void getCookies(final TaskExecutor.Callback<List<HttpCookie>> callback) {
        new Thread(() -> urlThread(callback)).start();
    }

    private void urlThread(TaskExecutor.Callback<List<HttpCookie>> callback) {
        ...
        if (callback != null) {
            ...
            if (canVisit) {
                try {
                    callback.onComplete(mCookieList);
